...
2. Для кожного елемента li.item у списку ul#categories знайде й виведе в консоль
   текст заголовка елемента (тегу _h2_) і кількість елементів у категорії (усіх
   _li_, вкладених у нього).
...
Використовуй масив об'єктів images для створення елементів _img_, вкладених в
_li_. Ти можеш створити й додати HTML-елементи, використовуючи
...
Напиши скрипт, який змінює колір фону елемента _body_ через інлайн-стиль по
...
умову, мають додаватися нові _div_ елементи в DOM. Для рендеру елементів на
сторінці створи функцію createBoxes(amount), яка приймає один параметр — число,
що зберігає кількість елементів для рендеру.

Функція має створювати стільки _div_ елементів, скільки вказано в параметрі
amount і додавати їх у DOM дочірніми елементами для div#boxes.

1. Розміри першого _div_ елемента мають бути 30px на 30px.
